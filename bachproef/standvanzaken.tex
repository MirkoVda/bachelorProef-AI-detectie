...
\begin{itemize}
  \item volledig AI-gegenereerde afbeeldingen (bv.~diffusion-output),
  \item partieel AI-gemanipuleerde afbeeldingen (bv.~generative fill, inpainting, face replacement).
\end{itemize}
...
Desondanks blijven CNN's relevant omwille van hun relatief lage rekenkost, maturiteit in tooling en goede prestaties in resource-beperkte omgevingen. In operationele settings met strikte latency-eisen vormen ze vaak een sterke baseline.
...
FaceForensics++ blijft een referentiebenchmark in veel studies en heeft het veld aanzienlijk vooruitgeholpen door gestandaardiseerde evaluatie mogelijk te maken~\autocite{Rossler2019}. Het zwakke punt is echter dat de dataset vooral oudere manipulatieprocedures omvat, waardoor performantie op deze benchmark geen garantie biedt voor actuele inzetbaarheid.
...
Ondanks sterke benchmarkresultaten blijft transfer naar onbekende of recente generatietechnieken een centraal probleem~\autocite{Hasan2026,Pei2024}. Veel modellen leren impliciet sporen van de trainingsbron in plaats van robuuste manipulatiekenmerken. Deze bachelorproef adresseert dat hiaat via datasetdiversiteit en een vergelijkende evaluatie over meerdere modeltypes.
